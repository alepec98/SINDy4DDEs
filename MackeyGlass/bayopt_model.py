...
class BayOpt(object):
    ...
    def run(self, max_iterations):
        parameter_space = ParameterSpace([DiscreteParameter('delay', range(25,475,5)), DiscreteParameter('exponent', range(13))])
...
delay_min = int(minimum[0])
n_min = int(minimum[1])

x_train_delay = x[delay_min:delay_min+n_snaps]
den = 1/(1+x_train_delay**n_min)
X = np.array([x_train, x_train_delay, den]).T
